...
**             For TT-TDB,  set ntarg = 17.  It looks as if                 **
**             ntarg = 16 has been skipped.)                                **
**                                                                          **
                      ...
                      const int ncent, double rrd[], const int calc_velocity)
{
  ...
  int i, k, list[13];    /* list is a vector denoting, for which "body"
...
/*   check for nutation call    */
   ...
   if( ntarg == 14)
      ...
      if( eph->ipt[11][1] > 0) /* there is nutation on ephemeris */
         ...
         rval = JPL_EPH_NO_NUTATIONS_IN_EPHEMERIS;
      return( rval);
      }
...
/*  check for librations */
   ...
   if( ntarg == 15)
      ...
      if( eph->ipt[12][1] > 0) /* there are librations on ephemeris file */
         ...
         rval = JPL_EPH_NO_LIBRATIONS_IN_EPHEMERIS;
      return( rval);
      }

   if( ntarg == 17)        /* check for TT-TDB */
      {
      if( eph->ipt[14][1] > 0) /* there is TT-TDB data in ephemeris file */
         {
         list[12] = list_val;
         rval = jpl_state( eph, et, list, pv, rrd, 0);
         }
      else          /*  no TT-TDB data in the ephemeris file  */
         rval = JPL_EPH_NO_TT_MINUS_TDB_DATA;
      return( rval);
      }
...
/*  be sure that at least 'ncf' polynomials have been evaluated
    and are stored in the array 'iinfo->posn_coeff'.    */
...
/*  if velocity interpolation is wanted, be sure enough
   ...
   iinfo->vel_coeff[2] = iinfo->twot + iinfo->twot;
...
**                                                                          **
*****************************************************************************/
int DLL_FUNC jpl_state( void *ephem, const double et, const int list[13],
...
/*   read correct record if not in core (static vector buf[])   */
   ...
   for( n_intervals = 1; n_intervals <= 8; n_intervals *= 2)
      for( i = 0; i < 11; i++)
         if( n_intervals == eph->ipt[i][2] && (list[i] || (i == 10 && recompute_pvsun)))
            {
            const unsigned flag = ((i == 10) ? 3 : list[i]);
            double *dest = ((i == 10) ? eph->pvsun : pv[i]);

            interp( &eph->iinfo, &buf[eph->ipt[i][0]-1], t, (int)eph->ipt[i][1], 3,
...
/*  do nutations if requested (and if on file)    */
   ...
   if( list[10] > 0 && eph->ipt[11][1] > 0)
      interp( &eph->iinfo, &buf[eph->ipt[11][0]-1], t, (int)eph->ipt[11][1], 2,
...
/*  get librations if requested (and if on file)    */
   ...
   if( list[11] > 0 && eph->ipt[12][1] > 0)
      ...
      interp( &eph->iinfo, &buf[eph->ipt[12][0]-1], t, (int)eph->ipt[12][1], 3,
                           (int)eph->ipt[12][2], list[11], pefau);
      for( j = 0; j < 6; ++j)
         pv[10][j] = pefau[j];
      }

/* get TT-TDB data if requested (and if on file) */
   if( list[12] > 0 && eph->ipt[14][1] > 0)
      interp( &eph->iinfo, &buf[eph->ipt[14][0]-1], t, (int)eph->ipt[14][1], 1,
                              (int)eph->ipt[14][2], list[12], nut);
                          ...
                          char nam[][6], double *val)
{
   ...
   rval->iinfo.n_posn_avail = 2;
   rval->iinfo.n_vel_avail = 3;
   rval->iinfo.posn_coeff[0] = 1.0;
   rval->iinfo.posn_coeff[1] = 0.0;
